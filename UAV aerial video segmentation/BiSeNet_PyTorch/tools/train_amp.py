...
# TODO: 使用Potsdam对应的 get_data_loader
# from lib.potsdam_dataset import get_data_loader
from lib.ussd_dataset import get_data_loader
...
#         net.load_state_dict(torch.load(args.finetune_from, map_location='cpu'))
    ...
    net.load_state_dict(torch.load('/root/BiSeNet_v1_v2_PyTorch/zyh/labs/ussd_v2_0113/model_final.pth', map_location='cpu'))
...
def train():
    ...
    for it, (im, lb) in enumerate(dl):
        ...
        if (it + 1) % 100 == 0:
            if not os.path.exists(cfg.respth):  # 为了能够在九天平台上手动停止
                print('手动终止')
                return
            lr = lr_schdr.get_lr()
            lr = sum(lr) / len(lr)
        ...
        if it % cfg.save_model_freq == 0:
            torch.save(state, osp.join(cfg.respth, f'iter-{it}.pth'))
