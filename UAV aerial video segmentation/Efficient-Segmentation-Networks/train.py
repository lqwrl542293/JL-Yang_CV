...
def parse_args():
    ...
    parser.add_argument('--train_type', type=str, default="trainval",
                        help="train for training on train set, trainval for training on train+val set")
    ...
    parser.add_argument('--optim', type=str.lower, default='adam',
                        choices=['sgd', 'adam', 'radam', 'ranger'], help="select optimizer")
...
#         criteria = ProbOhemCrossEntropy2d(use_weight=True, ignore_label=ignore_label, thresh=0.7, min_kept=min_kept)
    ...
    for epoch in range(start_epoch, args.max_epochs):
        ...
        # args.train_type为train而不是trainval时不进行val过程
        if args.train_type == 'trainval' and (epoch % 50 == 0 or epoch == (args.max_epochs - 1)):
