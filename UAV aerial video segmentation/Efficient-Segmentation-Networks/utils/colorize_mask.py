...
def ussd_colorize_mask(mask):
    # USSD RGB Color Map:
    COLOR_MAP = [(0, 0, 0), (255, 255, 0), (0, 0, 255), (0, 255, 0), (128, 0, 128), (255, 0, 0)]
    new_mask = np.zeros(shape=[mask.shape[0], mask.shape[1], 3], dtype=np.uint8)
    unique_label_ids = np.unique(mask)
    for label_id in unique_label_ids:
        idx = np.where(mask == label_id)
        new_mask[idx] = COLOR_MAP[label_id]
