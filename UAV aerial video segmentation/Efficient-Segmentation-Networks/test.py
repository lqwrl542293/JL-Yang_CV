...
def parse_args():
    ...
    parser.add_argument('--oa_kappa', action='store_true')
...
def test(args, test_loader, model):
    ...
    model.eval()    # ESPNet_v2使用eval mode将会出现错误
#     model.train()
    ...
    for i, (input, label, size, name) in enumerate(test_loader):
        ...
        if args.oa_kappa:
            global_y_true.append(gt)
            global_y_pred.append(output)
