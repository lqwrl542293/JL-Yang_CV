...
class hourglass(nn.Module):
    def __init__(self, inplanes):
        ...
        self.conv_1x1 = nn.Sequential(nn.Conv3d(inplanes*2,inplanes*2,kernel_size=1,bias=False),
        ...
        self.conv5 = nn.Sequential(nn.ConvTranspose3d(inplanes*4, inplanes*2, kernel_size=3, padding=1, output_padding=1, stride=2,bias=False),
    ...
    def forward(self, x):
        ...
        # else:
        pre = self.conv_1x1(pre)
        post = self.conv5(out)+pre
...
class PSMNet(nn.Module):
    def __init__(self, maxdisp):
        ...
        self.conv_1x1 = nn.Sequential(nn.Conv3d(32, 32, kernel_size=1,bias=False),
                                      nn.BatchNorm3d(32))
    ...
    def forward(self, left, right):
        ...
        add_cost0 = self.conv_1x1(cost0)
        out1 = self.dres2(cost0)
        out1 = out1+add_cost0
        out1 = F.relu(out1,inplace=True)

        add_cost1 = self.conv_1x1(out1)
        out2= self.dres3(out1)
        out2 = out2+add_cost1
        out2 = F.relu(out2, inplace=True)

        add_cost2 = self.conv_1x1(out2)
